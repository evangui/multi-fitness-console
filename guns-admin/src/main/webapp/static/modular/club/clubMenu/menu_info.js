...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/clubMenu/add", function (data) {
        Feng.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/clubMenu/edit", function (data) {
        Feng.success("修改成功!");
        window.parent.Menu.table.refresh();
        MenuInfoDlg.close();
    }, function (data) {
...
$(function () {
    ...
    var ztree = new $ZTree("pcodeTree", "/clubMenu/selectMenuTreeList");
