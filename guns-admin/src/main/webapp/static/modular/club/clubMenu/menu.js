...
Menu.openAddMenu = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/clubMenu/menu_add'
    });
...
Menu.openChangeMenu = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/clubMenu/menu_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Menu.delMenu = function () {
    if (this.check()) {
        ...
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/clubMenu/remove", function (data) {
                Feng.success("删除成功!");
                Menu.table.refresh();
            }, function (data) {
...
$(function () {
    ...
    var table = new BSTreeTable(Menu.id, "/clubMenu/list", defaultColunms);
