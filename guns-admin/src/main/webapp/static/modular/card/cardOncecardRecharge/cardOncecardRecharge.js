 ...
 * 次卡充值记录管理初始化
 */
var CardOncecardRecharge = {
    id: "CardOncecardRechargeTable",	//表格id
    seItem: null,		//选中的条目
    table: null,
    layerIndex: -1
};
 ...
 * 初始化表格的列
 */
CardOncecardRecharge.initColumn = function () {
    return [
 ...
 * 检查是否选中
 */
CardOncecardRecharge.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        CardOncecardRecharge.seItem = selected[0];
        return true;
    }
};
 ...
 * 点击添加次卡充值记录
 */
CardOncecardRecharge.openAddCardOncecardRecharge = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/cardOncecardRecharge/cardOncecardRecharge_add'
    });
 ...
 * 打开查看次卡充值记录详情
 */
CardOncecardRecharge.openCardOncecardRechargeDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '次卡充值记录详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/cardOncecardRecharge/cardOncecardRecharge_update/' + CardOncecardRecharge.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除次卡充值记录
 */
CardOncecardRecharge.delete = function () {
    if (this.check()) {
    	var operation = function(){
	        var ajax = new $ax(Feng.ctxPath + "/cardOncecardRecharge/delete", function (data) {
            Feng.success("删除成功!");
            CardOncecardRecharge.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("cardOncecardRechargeId",this.seItem.id);
            ajax.start();
    	}
 ...
 * @returns {{}}
 */
CardOncecardRecharge.formParams = function() {
	var queryData = {};
 ...
 * 查询次卡充值记录列表
 */
CardOncecardRecharge.search = function () {
    
    CardOncecardRecharge.table.refresh({query: CardOncecardRecharge.formParams()});
};

$(function () {
    var defaultColunms = CardOncecardRecharge.initColumn();
    var table = new BSTable(CardOncecardRecharge.id, "/cardOncecardRecharge/list", defaultColunms);
    table.setPaginationType("server");
    table.setQueryParams(CardOncecardRecharge.formParams());
    
    CardOncecardRecharge.table = table.init();
});
