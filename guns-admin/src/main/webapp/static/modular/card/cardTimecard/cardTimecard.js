 ...
 * 初始化表格的列
 */
CardTimecard.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '卡名', field: 'title', visible: true, align: 'center', valign: 'middle', sortable:true},
            {title: '卡种类型ID', field: 'cardTypeId', visible: true, align: 'center', valign: 'middle', sortable:true},
            ...
            {title: '有效期天数', field: 'expireDays', visible: true, align: 'center', valign: 'middle', sortable:true},
            ...
            {title: '请假开始时间', field: 'suspendStartTime', visible: true, align: 'center', valign: 'middle', sortable:true},
            {title: '请假截止时间', field: 'suspendEndTime', visible: true, align: 'center', valign: 'middle', sortable:true},
 ...
 * 检查是否选中
 */
CardTimecard.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        CardTimecard.seItem = selected[0];
        return true;
    }
};
 ...
 * 点击添加会员时间卡
 */
CardTimecard.openAddCardTimecard = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/cardTimecard/cardTimecard_add'
    });
 ...
 * 打开查看会员时间卡详情
 */
CardTimecard.openCardTimecardDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '会员时间卡详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/cardTimecard/cardTimecard_update/' + CardTimecard.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除会员时间卡
 */
CardTimecard.delete = function () {
    if (this.check()) {
    	var operation = function(){
	        var ajax = new $ax(Feng.ctxPath + "/cardTimecard/delete", function (data) {
            Feng.success("删除成功!");
            CardTimecard.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("cardTimecardId",this.seItem.id);
            ajax.start();
    	}
 ...
 * @returns {{}}
 */
CardTimecard.formParams = function() {
	var queryData = {};
 ...
 * 查询会员时间卡列表
 */
CardTimecard.search = function () {
    
    CardTimecard.table.refresh({query: CardTimecard.formParams()});
};

$(function () {
    var defaultColunms = CardTimecard.initColumn();
    var table = new BSTable(CardTimecard.id, "/cardTimecard/list", defaultColunms);
    table.setPaginationType("server");
    table.setQueryParams(CardTimecard.formParams());
    
    CardTimecard.table = table.init();
});
