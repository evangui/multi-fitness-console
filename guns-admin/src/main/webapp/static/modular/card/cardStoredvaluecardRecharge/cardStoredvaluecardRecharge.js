 ...
 * 储值卡充值记录管理初始化
 */
var CardStoredvaluecardRecharge = {
    id: "CardStoredvaluecardRechargeTable",	//表格id
    seItem: null,		//选中的条目
    table: null,
    layerIndex: -1
};
 ...
 * 初始化表格的列
 */
CardStoredvaluecardRecharge.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '总面额', field: 'sumMoney', visible: true, align: 'center', valign: 'middle'},
            {title: '已用面额', field: 'leftMoney', visible: true, align: 'center', valign: 'middle'},
 ...
 * 检查是否选中
 */
CardStoredvaluecardRecharge.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        CardStoredvaluecardRecharge.seItem = selected[0];
        return true;
    }
};
 ...
 * 点击添加储值卡充值记录
 */
CardStoredvaluecardRecharge.openAddCardStoredvaluecardRecharge = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/cardStoredvaluecardRecharge/cardStoredvaluecardRecharge_add'
    });
 ...
 * 打开查看储值卡充值记录详情
 */
CardStoredvaluecardRecharge.openCardStoredvaluecardRechargeDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '储值卡充值记录详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/cardStoredvaluecardRecharge/cardStoredvaluecardRecharge_update/' + CardStoredvaluecardRecharge.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除储值卡充值记录
 */
CardStoredvaluecardRecharge.delete = function () {
    if (this.check()) {
    	var operation = function(){
	        var ajax = new $ax(Feng.ctxPath + "/cardStoredvaluecardRecharge/delete", function (data) {
            Feng.success("删除成功!");
            CardStoredvaluecardRecharge.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("cardStoredvaluecardRechargeId",this.seItem.id);
            ajax.start();
    	}
 ...
 * @returns {{}}
 */
CardStoredvaluecardRecharge.formParams = function() {
	var queryData = {};
 ...
 * 查询储值卡充值记录列表
 */
CardStoredvaluecardRecharge.search = function () {
    
    CardStoredvaluecardRecharge.table.refresh({query: CardStoredvaluecardRecharge.formParams()});
};

$(function () {
    var defaultColunms = CardStoredvaluecardRecharge.initColumn();
    var table = new BSTable(CardStoredvaluecardRecharge.id, "/cardStoredvaluecardRecharge/list", defaultColunms);
    table.setPaginationType("server");
    table.setQueryParams(CardStoredvaluecardRecharge.formParams());
    
    CardStoredvaluecardRecharge.table = table.init();
});
